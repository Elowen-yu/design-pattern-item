...
//model
public class AdminEditPanel extends JInternalFrame {//所有编辑页面都可以用这个窗口
    public AdminEditPanel(JLabel[] labels, int number, Object selectedId) {
        ...
        if (id > -1){//修改
            //得到选中的数组 并放进文本框
            Object[] oldData = AdminDAO.findById(id);
        ...
        button.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(id <= -1){
                    AdminDAO.add(newData);
                    AdminPanel.setContent(new AdminTablePanel());
                }
                else {//修改
                    newData[0] = id;
                    AdminDAO.update(id,newData);
                    //更换页面
                    AdminPanel.setContent(new AdminTablePanel());
                }
            }
