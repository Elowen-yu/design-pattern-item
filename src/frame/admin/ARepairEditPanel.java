...
//model
public class ARepairEditPanel extends JInternalFrame {//所有编辑页面都可以用这个窗口
    public ARepairEditPanel(JLabel[] labels, int number, Object selectedId) {
...
//        //selectedId > -1 modify
        if (id > -1){//修改
            //得到选中的数组 并放进文本框
            Object[] oldData = ARepairDAO.findById(id);
        ...
        button.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(id <= -1){
                    ARepairDAO.add(newData);
                    AdminPanel.setContent(new ARepairPanel());
                }
                else {//修改
                    newData[0] = id;
                    ARepairDAO.update(id,newData);
                    //更换页面
                    AdminPanel.setContent(new ARepairPanel());
                }
            }
