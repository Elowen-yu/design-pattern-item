...
//model
public class RepairEditPanel extends JInternalFrame {//所有编辑页面都可以用这个窗口
    public RepairEditPanel(JLabel[] labels, int number, Object selectedId) {
...
//        //selectedId > -1 modify
        if (id > -1){//修改
            //得到选中的数组 并放进文本框
            Object[] oldData = ARepairDAO.findByIdUR(id);
        ...
        button.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(id <= -1){
                    ARepairDAO.addUR(newData);
                    ResPanel.setContent(new RepairPanel());
                }
                else {//修改
                    newData[0] = id;
                    ARepairDAO.updateUR(id,newData);
                    //更换页面
                    ResPanel.setContent(new RepairPanel());
                }
            }
